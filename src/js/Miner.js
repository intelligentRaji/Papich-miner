...
export class Miner extends BaseComponent {
    ...
    getCells() {
        return Array.from({ length: this.size }, (_, i) => Array.from({ length: this.size }, (q, j) => {
    ...
    subscribeCell(cell) {
        ...
        cell.subscribe("open", (row, column, mode) => this.calculations(row, column, mode));
    }
    getCellsFromLocalStorage(save) {
        return Array.from(save, (row1, i) => row1.map((cell, j) => {
            const element = new Cell({
                parent: this.element,
    ...
    toLocalStorage() {
        const save = this.cells.map((row) => row.map((cell) => cell.toLocalStorage()));
