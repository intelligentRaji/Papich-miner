...
export class Game extends BaseComponent {
    constructor(parent) {
        ...
        this.miner.cells.forEach((row) => row.forEach((cell) => {
    ...
    removeMissedFlag(coordinates) {
        if (!this.miner.cells[coordinates.i][coordinates.j].state.isBomb) {
            this.missedFlags = this.missedFlags.filter((element) => element.i !== coordinates.i && element.j !== coordinates.j);
        }
    }
    minusBombsLeft() {
    ...
    endGame() {
        ...
        this.miner.cells.forEach((row) => row.forEach((cell) => cell.removeListeners()));
    ...
    lose(cell) {
        ...
        this.missedFlags.forEach((flag) => this.miner.cells[flag.i][flag.j].openFlagAutomaticly());
        this.endGame();
    ...
    win() {
        if (this.cellsLeft === 0 && this.missedFlags.length === 0) {
            ...
            this.openModalWindow(`Ты разминировал поле за ${this.informationPanel.getTime()} и ${this.clicks.getValue()} ходов! ` +
